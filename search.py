...
def search(query, instance_url='http://localhost:8983/solr/collection_1/'):
    ...
    q_list = []
    if query:
        q_list.append('text:' + query + '^1.2')
    if pos_tag_data:
        q_list.append('pos_tag:' + pos_tag_data + '^0.02')
    if lemmas:
        q_list.append('lemmas:' + lemmas + '^1.7')
    if stems:
        q_list.append('stems:' + stems + '^1.5')
    if synonyms:
        q_list.append('synonyms:' + synonyms + '^2')
    if hypernyms:
        q_list.append('hypernyms:' + hypernyms + '^1.4')
    if hyponyms:
        q_list.append('hyponyms:' + hyponyms + '^1.4')
    if meronyms:
